...
with st.expander('Colunas'):
    colunas = st.multiselect('Selecione as colunas', options=dados.columns.tolist(), default=dados.columns.tolist())

st.sidebar.title('Filtros')
with st.sidebar.expander('Nome do produto'):
    produtos = st.multiselect('Selecione os produtos', options=dados['Produto'].unique(), default=dados['Produto'].unique())
with st.sidebar.expander('Preço do produto'):
    preco = st.slider('Selecione o preço', min_value=float(dados['Preço'].min()), max_value=float(dados['Preço'].max()), value=(float(dados['Preço'].min()), float(dados['Preço'].max())))
...
st.markdown(f"A tabela possui <span style='color:red;'>{dados_filtrados.shape[0]}</span> linhas e <span style='color:red;'>{dados_filtrados.shape[1]}</span> colunas.",
